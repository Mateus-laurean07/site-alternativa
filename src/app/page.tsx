...
export default function HomePage() {
  ...
  return (
    <>
      {/* HERO */}
      <section style={{ position: "relative", height: "100vh", minHeight: 600, display: "flex", alignItems: "center", background: current.bg, transition: "background 0.8s ease" }}>
        <div className="container" style={{ position: "relative", zIndex: 2 }}>
          <div style={{ maxWidth: 720 }}>
            <span className="badge badge-ouro" style={{ marginBottom: 24, display: "inline-block" }}>{current.tag}</span>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <Link href="/produtos" className="btn-ouro">{language === "PT" ? "Ver Produtos" : "View Products"}</Link>
              <Link href="/contato" className="btn-secondary" style={{ color: "white", borderColor: "rgba(255,255,255,0.5)" }}>
                {t("nav.quote")}
              </Link>
            </div>
          </div>
        ...
        <div style={{ position: "absolute", bottom: 40, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 10, zIndex: 2 }}>
          {heroSlides.map((_, i) => (
            <button key={i} onClick={() => setSlide(i)} style={{ width: i === slide ? 32 : 10, height: 10, borderRadius: 5, background: i === slide ? "var(--ouro)" : "rgba(255,255,255,0.4)", border: "none", cursor: "pointer", transition: "all 0.3s" }} />
          ))}
        </div>
        {/* Scroll hint */}
      ...
      {/* STATS */}
      <section style={{ background: "var(--gradient-verde)", padding: "48px 0" }}>
      ...
      <section className="section-padding" style={{ background: "var(--cinza-claro)" }}>
        <div className="container">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
            <div>
              <span className="badge">{language === "PT" ? "Custo-Benefício" : "Cost-Benefit"}</span>
              <h2 className="section-title" style={{ marginTop: 16 }}>{language === "PT" ? "Quem Calcula, Investe" : "Who Calculates, Invests"}</h2>
              <div className="divider-verde" />
              <p className="section-subtitle" style={{ marginBottom: 32 }}>
                {language === "PT" ? 
                  <>Um lote de 50 animais suplementados por 5 meses pode gerar <strong>R$ 6.975,00</strong> em gastos com nutrição. Sem cobertura no cocho, até <strong style={{ color: "var(--verde-medio)" }}>R$ 3.487,50</strong> podem ser desperdiçados pela chuva.</> : 
                  <>A lot of 50 animals supplemented for 5 months can generate <strong>R$ 6,975.00</strong> in nutrition costs. Without trough coverage, up to <strong style={{ color: "var(--verde-medio)" }}>R$ 3,487.50</strong> can be wasted by rain.</>}
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
                {[
                  { label: language === "PT" ? "Custo total com suplemento" : "Total supplement cost", valor: "R$ 6.975", cor: "var(--verde-escuro)" },
                  { label: language === "PT" ? "Potencial perda sem cobertura" : "Potential loss without coverage", valor: "R$ 3.487", cor: "#e53935" },
                  { label: language === "PT" ? "Economia com Protecocho" : "Savings with Protecocho", valor: language === "PT" ? "até 50%" : "up to 50%", cor: "var(--verde-medio)" },
                  { label: language === "PT" ? "Retorno do investimento" : "Return on investment", valor: language === "PT" ? "1° mês" : "1st month", cor: "var(--ouro)" },
                ].map((item) => (
                  <div key={item.label} style={{ background: "white", borderRadius: 12, padding: "20px 16px", boxShadow: "var(--shadow-sm)", border: "1px solid var(--cinza-medio)" }}>
                    <div style={{ fontSize: "1.5rem", fontWeight: 800, color: item.cor, fontFamily: "Playfair Display, serif" }}>{item.valor}</div>
                    <div style={{ fontSize: "0.8rem", color: "var(--cinza-texto)", marginTop: 4 }}>{item.label}</div>
                  </div>
                ))}
              </div>
              <Link href="/produtos?categoria=Protecocho" className="btn-primary">{language === "PT" ? "Ver Linha Protecocho" : "View Protecocho Line"}</Link>
            </div>
            <motion.div 
              ...
              >
                ...
                <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 12, padding: 24, marginBottom: 24 }}>
                  <div style={{ fontSize: "0.875rem", color: "rgba(255,255,255,0.7)", marginBottom: 8 }}>{language === "PT" ? "Com cobertura Alternativa você protege:" : "With Alternativa coverage you protect:"}</div>
                  <div style={{ fontSize: "2.5rem", fontWeight: 900, color: "var(--ouro)", fontFamily: "Playfair Display, serif" }}>100%</div>
                  <div style={{ fontSize: "0.875rem", color: "rgba(255,255,255,0.7)" }}>{language === "PT" ? "do seu investimento em suplementação" : "of your supplement investment"}</div>
                </div>
                <Link href="/contato" className="btn-ouro" style={{ display: "inline-flex" }}>{t("nav.quote")}</Link>
              </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <span className="badge">{language === "PT" ? "Linha Completa" : "Complete Line"}</span>
            <h2 className="section-title" style={{ marginTop: 16 }}>{language === "PT" ? "Nossos Produtos em Destaque" : "Our Featured Products"}</h2>
            <div className="divider-verde" style={{ margin: "16px auto 24px" }} />
            <p className="section-subtitle" style={{ margin: "0 auto" }}>
              {language === "PT" ? "Do armazenamento à alimentação e hidratação — soluções completas para o manejo do seu rebanho." : "From storage to feeding and hydration — complete solutions for managing your herd."}
            </p>
          </div>
            ...
            {produtosDestaque.map((p) => (
              <Link key={p.id} href={`/produtos/${p.slug}`} className="produto-card" style={{ textDecoration: "none" }}>
                ...
                <div className="produto-card-body">
                  <div style={{ fontSize: "0.75rem", color: "var(--verde-medio)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>{p.categoria}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--verde-medio)", fontWeight: 600, fontSize: "0.9rem" }}>
                    {language === "PT" ? "Ver detalhes" : "View details"} <span style={{ fontSize: "1.1rem" }}>→</span>
                  </div>
                </div>
              </Link>
            ))}
          ...
          <div style={{ textAlign: "center", marginTop: 48 }}>
            <Link href="/produtos" className="btn-secondary">{language === "PT" ? "Ver Todos os Produtos" : "View All Products"}</Link>
          </div>
        </div>
      </section>

      {/* SOBRE */}
      <section className="section-padding" style={{ background: "var(--verde-escuro)" }}>
        <div className="container">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
            <motion.div 
              ...
              viewport={{ once: true }}
              style={{ position: "relative", height: "100%", minHeight: 400, borderRadius: 24, overflow: "hidden" }}
            >
              <Image 
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <span className="badge badge-ouro">{language === "PT" ? "Nossa História" : "Our History"}</span>
              <h2 style={{ color: "white", marginTop: 16, marginBottom: 16 }}>{language === "PT" ? "Desde 2011, Parceiro Ideal dos Criadores" : "Since 2011, the Ideal Partner for Breeders"}</h2>
              <div className="divider-ouro" />
              <p style={{ color: "rgba(255,255,255,0.75)", lineHeight: 1.8, marginBottom: 24 }}>
                {language === "PT" ? "A Alternativa Plásticos nasceu em Lucas do Rio Verde/MT com uma missão clara: oferecer soluções que promovam durabilidade, bem-estar animal e cuidado com o meio ambiente." : "Alternativa Plásticos was born in Lucas do Rio Verde/MT with a clear mission: to offer solutions that promote durability, animal welfare, and environmental care."}
              </p>
              <p style={{ color: "rgba(255,255,255,0.75)", lineHeight: 1.8, marginBottom: 40 }}>
                {language === "PT" ? "Pioneira em cochos plásticos de alta resistência, acompanhamos a evolução do agronegócio por mais de 14 anos, atendendo produtores em todo o Brasil." : "Pioneer in high-resistance plastic troughs, we have followed the evolution of agribusiness for over 14 years, serving producers throughout Brazil."}
              </p>
              <Link href="/sobre" className="btn-ouro">{language === "PT" ? "Conheça Nossa História" : "Discover Our History"}</Link>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <span className="badge">{language === "PT" ? "Diferenciais" : "Differentials"}</span>
            <h2 className="section-title" style={{ marginTop: 16 }}>{language === "PT" ? "Por que Escolher a Alternativa?" : "Why Choose Alternativa?"}</h2>
          ...
          >
            {features.map((f) => (
              <motion.div key={f.titulo} variants={fadeInUp} className="feature-card">
                <div className="feature-icon">{f.icon}</div>
                <h4 style={{ color: "var(--verde-escuro)", marginBottom: 10 }}>{f.titulo}</h4>
      ...
      <section className="section-padding" style={{ background: "var(--cinza-claro)" }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <span className="badge">{language === "PT" ? "Clientes Satisfeitos" : "Satisfied Customers"}</span>
            <h2 className="section-title" style={{ marginTop: 16 }}>{language === "PT" ? "O que dizem nossos clientes" : "What our customers say"}</h2>
            <div className="divider-verde" style={{ margin: "16px auto 0" }} />
          </div>
            ...
            {depoimentos.map((d) => (
              <div key={d.nome} className="testimonial-card">
                <div style={{ paddingTop: 24 }}>
                  <div style={{ display: "flex", gap: 4, marginBottom: 16, color: "var(--ouro)", fontSize: "1.1rem" }}>
                    {"★".repeat(d.stars)}
                  </div>
                  <p style={{ color: "var(--cinza-texto)", lineHeight: 1.7, marginBottom: 24, fontStyle: "italic" }}>
                    "{d.texto}"
                  </p>
                  <div style={{ fontWeight: 700, color: "var(--verde-escuro)" }}>{d.nome}</div>
                  <div style={{ fontSize: "0.85rem", color: "var(--cinza-texto)" }}>{d.local}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section-padding">
        <div className="container">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 48 }}>
            <div>
              <span className="badge">{language === "PT" ? "Conhecimento Agro" : "Agro Knowledge"}</span>
              <h2 className="section-title" style={{ marginTop: 16 }}>{language === "PT" ? "Últimas do Blog" : "Latest from the Blog"}</h2>
              <div className="divider-verde" />
            </div>
            <Link href="/blog" className="btn-secondary">{language === "PT" ? "Ver todos os posts" : "View all posts"}</Link>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
            {blogPosts.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`} className="blog-card" style={{ textDecoration: "none" }}>
                ...
                <div style={{ padding: 24 }}>
                  <span style={{ fontSize: "0.75rem", color: "var(--verde-medio)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em" }}>{post.categoria}</span>
                  <h4 style={{ color: "var(--verde-escuro)", margin: "8px 0 12px", fontFamily: "Playfair Display, serif" }}>{post.titulo}</h4>
                  <p style={{ fontSize: "0.875rem", color: "var(--cinza-texto)", marginBottom: 16 }}>{post.resumo}</p>
                  <div style={{ fontSize: "0.8rem", color: "var(--cinza-texto)" }}>{post.tempoLeitura} {language === "PT" ? "min de leitura" : "min read"}</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      {/* MAPA */}
      <section className="section-padding-sm" style={{ background: "var(--cinza-claro)" }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <span className="badge">{language === "PT" ? "Localização" : "Location"}</span>
            <h2 className="section-title" style={{ marginTop: 16 }}>{language === "PT" ? "Onde Estamos" : "Where We Are"}</h2>
            <p className="section-subtitle" style={{ margin: "16px auto 0" }}>BR 163, KM 660 — Comunidade São Cristóvão, Lucas do Rio Verde/MT</p>
          </div>
          <div style={{ borderRadius: 20, overflow: "hidden", boxShadow: "var(--shadow-lg)", height: 400 }}>
            <iframe
      ...
      <section style={{ background: "var(--gradient-verde)", padding: "80px 24px", textAlign: "center" }}>
        <div className="container">
          <h2 style={{ color: "white", marginBottom: 16 }}>{language === "PT" ? "Pronto para Transformar sua Propriedade?" : "Ready to Transform Your Property?"}</h2>
          <p style={{ color: "rgba(255,255,255,0.8)", fontSize: "1.1rem", marginBottom: 40, maxWidth: 560, margin: "0 auto 40px" }}>
            {language === "PT" ? "Entre em contato e receba um orçamento personalizado para as necessidades do seu rebanho." : "Contact us and receive a personalized quote for your herd's needs."}
          </p>
          <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/contato" className="btn-ouro">{t("nav.quote")}</Link>
            <a href="https://wa.me/5565999902024" target="_blank" rel="noreferrer" className="btn-whatsapp">
